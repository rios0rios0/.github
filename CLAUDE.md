...
There is no build, test, or lint tooling in this repo. Changes are validated by inspection — verify YAML syntax, confirm `workflow-templates/*.yml` still have their paired `*.properties.json`, and ensure reusable workflows still accept their declared inputs.
