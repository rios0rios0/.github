...
2. **Workflow templates** — files in `workflow-templates/` appear in the **Actions → New workflow** picker across all `rios0rios0` repos. Each workflow template (`*.yml`/`*.yaml`) has a paired `*.properties.json` (name/description/icon/categories). All templates are thin wrappers that call reusable workflows from [`rios0rios0/pipelines`](https://github.com/rios0rios0/pipelines): the language templates pin a major tag (`uses: rios0rios0/pipelines/.github/workflows/<lang>-docker.yaml@v3`), while the Claude templates track `@main` like the rest of the account's first-party references to that repository. Each `workflow-templates/*.yaml` keeps its paired `*.properties.json`, so renaming one renames both. The reusable Claude Code workflows now live in [`rios0rios0/pipelines`](https://github.com/rios0rios0/pipelines) as `reusable-claude-review.yaml` and `reusable-claude-mention.yaml`; the `reusable-` prefix marks a definition, so `workflow-templates/claude*.yaml` and this repo's own `.github/workflows/claude*.yaml` drop it and are named `claude-review.yaml` / `claude-mention.yaml` — the same names a consuming repository uses. They are callers, and pass `CLAUDE_CODE_OAUTH_TOKEN` explicitly rather than with `secrets: inherit`.
...
There is no build, test, or lint tooling in this repo. Changes are validated by inspection — verify YAML syntax, confirm `workflow-templates/*.yml` and `workflow-templates/*.yaml` still have their paired `*.properties.json`, and ensure every `uses:` names a reusable workflow that exists in `rios0rios0/pipelines`.
