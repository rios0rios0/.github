---
name: code-review
description: "Review pull requests and diffs in the .github repository — the community-health defaults and workflow templates inherited by every rios0rios0 repository — against the rios0rios0/guide standards, with extra weight on the blast radius of fallback files, action pinning, and template pairing. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `.github`

This repository supplies the fallback community-health files (`CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, `LICENSE`, PR and issue templates, `FUNDING.yml`, `.editorconfig`) inherited by every `rios0rios0` repository that lacks its own, plus the workflow templates that appear in the Actions picker. Every edit here is visible across the whole account.
...
```bash
# no build system — validate by rendering the template in a scratch repository
yamllint .github/workflows/*.yaml     # or the equivalent your editor provides
```
...
There is no test tooling. Verification is reading the change at org scale and, for a workflow template, running it once in a scratch repository before merging.
...
- The grandfathered `.yml` extensions under `workflow-templates/`.
- The absence of a build or test system — this repository holds configuration only.
